...
(() => {
  ...
  function bridgePath(l) {
    const left = l.leftHairX + 14, right = l.rightHairX - 14, top = l.circleY - 96, bottom = l.circleY + 92;
    return `M ${left} ${top} C ${left + 18} ${top + 65}, ${left + 24} ${bottom - 58}, ${left + 17} ${bottom} C ${left + 47} ${bottom - 22}, ${right - 47} ${bottom - 22}, ${right - 17} ${bottom} C ${right - 24} ${bottom - 58}, ${right - 18} ${top + 65}, ${right} ${top} C ${right - 38} ${top + 45}, ${left + 38} ${top + 45}, ${left} ${top} Z`;
  }
  function transferPath(l) {
    const cx = (l.leftHairX + l.rightHairX) / 2, bridgeBottom = l.circleY + 92, bottom = l.substrateY;
    return `M ${cx - 23} ${bridgeBottom - 5} C ${cx - 20} ${bridgeBottom + 57}, ${cx - 18} ${bottom - 54}, ${cx - 43} ${bottom - 10} C ${cx - 51} ${bottom - 4}, ${cx - 58} ${bottom - 1}, ${cx - 62} ${bottom} L ${cx + 62} ${bottom} C ${cx + 58} ${bottom - 1}, ${cx + 51} ${bottom - 4}, ${cx + 43} ${bottom - 10} C ${cx + 18} ${bottom - 54}, ${cx + 20} ${bridgeBottom + 57}, ${cx + 23} ${bridgeBottom - 5} C ${cx + 12} ${bridgeBottom + 12}, ${cx - 12} ${bridgeBottom + 12}, ${cx - 23} ${bridgeBottom - 5} Z`;
  }
  function run() {
    ...
    later(() => {
      ...
      later(() => { fade(question, 0, 150); const gap = sketchText('gap', C.text.gap, l.labels.gap); show(gap, 130, C.timings.gap.duration); }, C.timings.gap.start - C.timings.circle.start);
    ...
    later(() => {
      ...
      later(() => { fade(el('annotation-layer').querySelector('.bridge-note'), 0, 180); const note = sketchText('transfer-note', C.text.transfer, l.labels.transfer, 'transfer'); show(note, 0, C.timings.transferLabel.duration); }, C.timings.transferLabel.start - C.timings.transfer.start);
